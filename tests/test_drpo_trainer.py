...
class TestDRPO(unittest.TestCase):
    def setUp(self):
        ...
        self.dummy_dataset = Dataset.from_dict({
            "prompt": ["Summarize this:", "Explain this:"] * 2,
            "a1": ["Summary A", "Explanation A"] * 2,
            "a2": ["Summary B", "Explanation B"] * 2,
            "rank": [1.0, 1.0] * 2
        })
    ...
    def test_drpo_trainer_loss_consistency_a(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            def get_loss_for_mode(mode=None):
                ...
                args.model_and_preference_share_basemodel = True
                preference_model = BTRewardNetwork(self.preference_model_id, pad_token_id=self.tokenizer.pad_token_id)
    ...
    def test_drpo_trainer_loss_consistency_b(self):
        """Test loss consistency across different modes."""
        print("========================\n WITH decoding \n========================")
        with tempfile.TemporaryDirectory() as tmp_dir:
            def get_loss_for_mode(mode=None):
                args = deepcopy(self.base_training_args)
                args.output_dir = tmp_dir
                args.model_and_preference_share_basemodel = False
                preference_model = BTwithRewardPipeline(self.preference_model_id, self.model_id)
