...
class BTRewardNetwork(nn.Module):
    def __init__(self, model_name_or_path: str, revision: str = "main", device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu"), pad_token_id: Optional[int]=None):
        super().__init__()
        self.rm = AutoModelForSequenceClassification.from_pretrained(
            model_name_or_path,
            revision=revision,
