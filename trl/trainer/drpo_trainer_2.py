...
class DRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        self.mu_feature_dim: Optional[int] = None

        if self.learn_mu_parameters:
            self.mu_feature_dim = self._resolve_mu_feature_dim(model)
            ...
            needs_mu_ref = (
                "mu_ref" not in model.mu_value_networks
                or not isinstance(model.mu_value_networks["mu_ref"], nn.Sequential)
                or getattr(model.mu_value_networks["mu_ref"][0], "in_features", None) != self.mu_feature_dim
            )
            needs_mu_theta = (
                "mu_theta" not in model.mu_value_networks
                or not isinstance(model.mu_value_networks["mu_theta"], nn.Sequential)
                or getattr(model.mu_value_networks["mu_theta"][0], "in_features", None) != self.mu_feature_dim
            )

            if needs_mu_ref:
                model.mu_value_networks["mu_ref"] = self._build_mu_value_network(self.mu_feature_dim, self.mu_value_hidden_size)
            if needs_mu_theta:
                model.mu_value_networks["mu_theta"] = self._build_mu_value_network(self.mu_feature_dim, self.mu_value_hidden_size)
    ...
    def _resolve_mu_feature_dim(self, model: PreTrainedModel) -> int:
        config = getattr(model, "config", None)
        if config is None:
            raise ValueError("Model configuration is required to initialize mu value networks.")
        ...
        if hidden_size is None:
            raise ValueError("Unable to infer hidden size for mu value networks from model configuration.")

        # We append one feature slot for the scalar bias term.
    ...
    def _calculate_kl_divergence(self, target_model, ref_model, prompt_ids, prompt_attention_mask, completion_ids, completion_attention_mask, temperature=1.0):
        # Get the number of tokens to truncate from prompt
        num_tokens_to_truncate = max(prompt_ids.size(1) + completion_ids.size(1) - self.max_length, 0)
        ...
        if args.model_and_preference_share_basemodel:
            ...
            if args.ratio_processing == "clip":
                ...
                if self.learn_mu_parameters:
                    ...
                    if self.mu_feature_dim is None:
                        raise RuntimeError("mu_feature_dim must be initialized when learn_mu_parameters is True.")
                    ...
                    with torch.no_grad():
                        ...
                        prompt_lengths = prompt_attention_mask.long().sum(dim=1) - 1
                        prompt_lengths = prompt_lengths.clamp_min(0)
                        batch_indices = torch.arange(
                            prompt_hidden_states.size(0), device=prompt_hidden_states.device
                        )
                        prompt_last_hidden = prompt_hidden_states[batch_indices, prompt_lengths]
                        del prompt_outputs

                    prompt_last_hidden = prompt_last_hidden.to(mu_value_dtype)

                    mu_ref_features = torch.cat((prompt_last_hidden, bias_device.unsqueeze(-1)), dim=-1)
